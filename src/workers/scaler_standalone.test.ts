...
describe('ContrastAwareScaler Standalone', () => {
    ...
    it('should run 100x100 -> 10x10 without hanging', () => {
        const input = createNoiseImage(100, 100);
        const start = performance.now();
        const result = processContrastAwareBase(input, 10, 10, 0);
        const end = performance.now();
        console.log(`Execution time (Downscale): ${end - start}ms`);
        expect(result).toBeDefined();
    });

    it('should run 100x100 -> 200x200 (Upscale) without hanging', () => {
        const input = createNoiseImage(100, 100);
        const start = performance.now();
        const result = processContrastAwareBase(input, 200, 200, 0);
        const end = performance.now();
        console.log(`Execution time (Upscale): ${end - start}ms`);
        expect(result).toBeDefined();
    });
});
